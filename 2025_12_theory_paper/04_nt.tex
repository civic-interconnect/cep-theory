...
fields. They impose a structural layer that is functorial: every payload
can be given an envelope, and every valid update to a payload induces a
corresponding update to its envelope. This section formalizes that layer
...
objects are un-enveloped CEP payloads (entities, relationships, and
...
mechanism). As with $\mathcal{E}$, this functor acts on both objects and
morphisms.
...
non-decreasing relation. Revision numbers in CEP form a functor:
\[
...
$(\mathbb{N}, \le)$. This captures the immutability and forward-only
...
interpretation: $\mathcal{E}$ behaves as a \emph{comonad} on the
category of payloads~\citep{maclane1971categories,awodey2010category}.

A comonad $(\mathcal{E}, \varepsilon, \delta)$ consists of:
\begin{itemize}
    \item a functor $\mathcal{E} : \mathbf{P} \to \mathbf{P}$,
    \item a counit $\varepsilon : \mathcal{E} \Rightarrow \mathrm{Id}$,
    \item a comultiplication
          $\delta : \mathcal{E} \Rightarrow \mathcal{E}\mathcal{E}$,
\end{itemize}
satisfying standard coassociativity and counitality laws.

In CEP:
\begin{itemize}
    \item $\mathcal{E}(P)$ is the ``contextualized'' version of $P$,
    \item $\varepsilon$ extracts the underlying payload,
    \item $\delta$ enriches a record with its own envelope again,
          representing recursive contextualization.
\end{itemize}
...
record-level consistency. CEP attestations must preserve pullbacks:
...
\begin{itemize}
    ...
    \item no new contradictions can arise under $\alpha$,
...
Let $\mathbf{Sch}$ denote the category of CEP schemas and vocabulary
URIs. Each CEP record is typed by a schema element, giving rise to a
functor:
\[
...
\begin{itemize}
    ...
    \item vocabulary evolution propagates along cartesian liftings.
\end{itemize}
...
\begin{center}
    \begin{tabular}{ll}
        ...
        \texttt{proofValue}           & \texttt{prov:wasGeneratedBy}                   \\
        \bottomrule
    \end{tabular}
\end{center}

Categorically, PROV's ``wasGeneratedBy'' and ``wasAttributedTo'' become
morphisms in a provenance category $\mathbf{Prov}$. The attestation
natural transformation $\alpha$ factors through a functor:
\[
